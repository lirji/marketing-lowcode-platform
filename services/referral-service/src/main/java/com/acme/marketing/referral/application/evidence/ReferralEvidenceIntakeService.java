...
public class ReferralEvidenceIntakeService {
    ...
    private Result acceptOutside(TenantScope scope,Envelope envelope) {
        ...
        for(int attempt=0;attempt<3;attempt++) {
            ...
            if(previous!=null && !conflict) {
                ...
                try{return replay(event,incoming,order,input);}catch(RetryPreparation retry){continue;}
            }
            ...
            if(conflict) {
                ...
                catch(RetryPreparation retry){continue;}
            ...
            try{return apply(scope,envelope,event,order,incoming,observed,priorHistory,prepared,nextSealed,changed,input);}catch(RetryPreparation retry){/* 下一轮在事务外重新准备。 */}
        }
    ...
    private Result replay(EventKey event,Sealed incoming,OrderKey order,VerifiedInput input) {
        return transactions.execute(tx->{anchor(event.tenantId(),input.keyVersion());Inbox locked=repository.lockInbox(event,incoming.businessDigest(),order,clock.instant());
            if(!locked.businessDigest().equals(incoming.businessDigest()) || !locked.originalOrder().equals(order) || locked.result()==null)throw new RetryPreparation();return locked.result();});
    }
    private Result apply(TenantScope scope,Envelope envelope,EventKey event,OrderKey order,Sealed incoming,StoredOrder observed,Sealed priorHistory,
        ...
        return transactions.execute(tx->{
            ...
            if(!receipt.businessDigest().equals(incoming.businessDigest()) || !receipt.originalOrder().equals(order))throw new RetryPreparation();
            ...
            if(!same(observed,locked) || !prepared.mayCommit(lockedVersion(locked,history,incoming.header().revision()),anchor,now))throw new RetryPreparation();
    ...
    private void quarantineConflict(TenantScope scope,Envelope envelope,EventKey event,Inbox previous,StoredOrder observed,Sealed next,VerifiedInput input) {
        transactions.executeWithoutResult(tx->{
            ...
            if(!lockedReceipt.equals(previous))throw new RetryPreparation();
            StoredOrder locked=repository.lockOrder(previous.originalOrder(),UUID.randomUUID().toString(),clock.instant());
            Instant now=clock.instant();
            if(!same(observed,locked) || !current(input,now) || locked.state().header().keyVersion()!=input.keyVersion())throw new RetryPreparation();
    ...
    private static final class RetryPreparation extends RuntimeException { @java.io.Serial private static final long serialVersionUID=1L; }
}
