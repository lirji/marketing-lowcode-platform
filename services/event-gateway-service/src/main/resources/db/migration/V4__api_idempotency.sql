CREATE TABLE mk_api_command (
  ...
  CONSTRAINT ck_event_api_command_state CHECK (state_name IN ('PROCESSING','COMPLETED'))
) COMMENT='Event Gateway写接口统一幂等记录（负载绑定与首次响应重放）';

CREATE INDEX ix_event_api_command_expiry ON mk_api_command (expires_at);
