...
public class BenefitOutboxPublisher {
    private final BenefitOutboxRepository repository;
    ...
    public BenefitOutboxPublisher(BenefitOutboxRepository repository, KafkaTemplate<String, String> kafka,
            ObjectMapper mapper,
    ...
    public Result publishBatch(int limit) {
        ...
        List<BenefitOutboxRepository.PendingEvent> rows = repository.lockPublishable(format(now), limit);
        ...
        for (Send send : sends) {
            if (send.future().isDone() && !send.future().isCompletedExceptionally()
                ...
                repository.markPublished(send.row().tenantId(), send.row().eventId(),
                        format(now), send.row().attempts() + 1);
                published.increment();
                success++;
            } else {
    ...
    private boolean recordFailure(BenefitOutboxRepository.PendingEvent row, Throwable cause, Instant now) {
        ...
        if (attempts >= maxAttempts) {
            repository.markDead(row.tenantId(), row.eventId(), format(now), attempts, reason);
            kafka.send(row.topic() + ".dlq", row.key(), deadLetter(row, reason, attempts));
            dead.increment();
            return true;
        }
        long delay = Math.min(60, 1L << Math.min(6, attempts - 1));
        repository.markRetry(row.tenantId(), row.eventId(), format(now.plusSeconds(delay)), attempts, reason);
        return false;
    }

    private String deadLetter(BenefitOutboxRepository.PendingEvent row, String reason, int attempts) {
        try {
    ...
    private record Send(BenefitOutboxRepository.PendingEvent row,
