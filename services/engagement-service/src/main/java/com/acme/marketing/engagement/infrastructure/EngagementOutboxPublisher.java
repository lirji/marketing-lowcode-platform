...
public class EngagementOutboxPublisher {
    ...
    public EngagementOutboxPublisher(JdbcTemplate jdbc, KafkaTemplate<String, String> kafka, ObjectMapper mapper,
    ...
    public Result publishBatch(int limit) {
        ...
        List<Row> rows = jdbc.query("select candidate.tenant_id,candidate.event_id,candidate.destination_topic,candidate.partition_key,candidate.payload_json,candidate.publish_attempts from mk_engagement_outbox candidate where candidate.published_at is null and candidate.dead_lettered_at is null and candidate.next_attempt_at<=? and not exists (select 1 from mk_engagement_outbox predecessor where predecessor.tenant_id=candidate.tenant_id and predecessor.contact_id=candidate.contact_id and predecessor.published_at is null and predecessor.dead_lettered_at is null and predecessor.stream_sequence<candidate.stream_sequence) order by candidate.created_at,candidate.stream_sequence limit ? for update skip locked",
        ...
        for (Send send : sends) {
            if (send.future().isDone() && !send.future().isCompletedExceptionally()
                ...
                jdbc.update("update mk_engagement_outbox set published_at=?,publish_attempts=?,last_error='' where tenant_id=? and event_id=? and published_at is null and dead_lettered_at is null",
    ...
    private boolean recordFailure(Row row, Throwable cause, Instant now) {
        ...
        if (attempts >= maxAttempts) {
            jdbc.update("update mk_engagement_outbox set publish_attempts=?,dead_lettered_at=?,last_error=? where tenant_id=? and event_id=? and published_at is null and dead_lettered_at is null",
        ...
        jdbc.update("update mk_engagement_outbox set publish_attempts=?,next_attempt_at=?,last_error=? where tenant_id=? and event_id=? and published_at is null",
