...
import { Ban, CheckCircle2, RotateCcw, Rocket, ShieldAlert } from 'lucide-react'
...
export function ReleaseCenterPage() {
  ...
  const activate = useMutation({
    mutationFn: (manifestId: string) => api.activateRelease(manifestId),
  ...
  const closeConfirm = () => { setConfirm(null); setReason(''); setChecked(false); void queryClient.invalidateQueries({ queryKey: ['releases'] }) }
  const actionError = activate.error ?? rollback.error ?? kill.error
  const canExecute = Boolean(latest) && checked && reason.trim().length >= 4 && !api.demoMode
  ...
  return <div className="workspace release-page">
    <PageHeader eyebrow={latest ? `RELEASE · ${latest.manifest.manifestId}` : 'RELEASE CENTER'} title={latest ? `${latest.manifest.cell} / ${latest.manifest.namespace ?? 'ns'}` : '发布中心'} description="签名 Manifest 是 desired state；所有 Cell 验签、预热并 ACK 后，流量才会进入新代际。" actions={<>
      <Button onClick={() => setConfirm('rollback')} disabled={!latest || !auth.hasPermission('release:rollback')} title={auth.hasPermission('release:rollback') ? '回滚' : '缺少 release:rollback'}><RotateCcw size={15} />回滚</Button>
      <Button tone="danger" onClick={() => setConfirm('kill')} disabled={!latest || !auth.hasPermission('release:kill-switch')} title={auth.hasPermission('release:kill-switch') ? 'Kill switch' : '缺少 release:kill-switch'}><Ban size={15} />Kill switch</Button>
      <Button tone="primary" onClick={() => setConfirm('activate')} disabled={!latest || !auth.hasPermission('release:activate')} title={auth.hasPermission('release:activate') ? '推进灰度' : '缺少 release:activate'}><Rocket size={15} />推进灰度</Button>
    </>} />
    <DemoBanner />
    ...
    {!api.demoMode && (releases.data ?? []).length === 0 && !releases.isLoading ? <EmptyState title="还没有 Release Manifest" detail="通过已批准定义编译并 stage 后，才会出现在此矩阵。" /> : null}
    ...
    {confirm && latest && (
      <Modal title={confirm === 'kill' ? '启用全局 Kill switch' : confirm === 'rollback' ? `回滚 ${latest.manifest.manifestId}` : `激活 ${latest.manifest.manifestId}`} description="此操作将写入不可篡改审计，并由 Release Coordinator 执行。" onClose={() => closeConfirm()}>
        <div className="danger-confirm">
          <StateBanner tone={confirm === 'kill' ? 'error' : 'warn'} title="确认作用范围" detail={`${latest.manifest.environment ?? ''} · ${latest.manifest.cell} · ${latest.manifest.runtime} · ${latest.manifest.namespace}`} />
          ...
          <footer className="modal-actions">
            ...
            <Button tone={confirm === 'kill' ? 'danger' : 'primary'} disabled={!canExecute || activate.isPending || rollback.isPending || kill.isPending} title={api.demoMode ? '演示模式不可执行' : '确认执行'} onClick={() => {
              if (!canExecute || !latest) return
              if (confirm === 'activate') activate.mutate(latest.manifest.manifestId)
              else if (confirm === 'rollback') rollback.mutate({ manifestId: latest.manifest.manifestId, generation: Math.max(1, (latest.manifest.stableGeneration ?? latest.manifest.generation) - 1 || 1) })
              else kill.mutate({ namespace: latest.manifest.namespace ?? 'main', reason })
            }}>确认执行</Button>
          </footer>
        </div>
      </Modal>
