...
export type DesignerNode = Node<{ label: string; subtitle: string; tone: 'teal'|'blue'|'amber'|'violet'|'slate'; config: Record<string, string>; stableTypeId?: string }, 'marketing'>
type Snapshot = { nodes: DesignerNode[]; edges: Edge[] }

function MarketingNode({ data, selected }: NodeProps<DesignerNode>) {
  return <div className={`flow-node ${data.tone} ${selected ? 'selected' : ''}`} tabIndex={0}><Handle type="target" position={Position.Left} /><span>{data.label.slice(0, 1)}</span><div><strong>{data.label}</strong><small>{data.subtitle}</small></div><Handle type="source" position={Position.Right} /></div>
}
...
export function LowCodeDesigner({
  ...
  return <div className="designer-page">
    ...
    <div className="designer-grid">
      <aside className="node-palette"><div className="designer-section-title"><span>节点与模板</span><small>{paletteItems.length} 个可用</small></div><label className="palette-search"><span className="sr-only">搜索节点</span><input placeholder="搜索节点…" /></label>{paletteItems.map((item) => <button key={item.stableTypeId ?? item.label} onClick={() => addNode(item)}><i className={item.tone}>{item.label.slice(0, 1)}</i><div><strong>{item.label}</strong><small>{item.subtitle}</small></div><ChevronRight size={13} /></button>)}<div className="complexity-meter"><div><span>复杂度预算</span><b>68 / 100</b></div><progress value="68" max="100" /></div></aside>
