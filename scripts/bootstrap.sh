...
if [[ "${with_observability}" == true ]]; then
  ...
  # 本地显式开启观测时默认全采样，确保一次联调请求可被定位；压测时由调用方主动降采样。
  export TRACING_SAMPLE_PROBABILITY="${TRACING_SAMPLE_PROBABILITY:-1.00}"
  export DECISION_TRACING_SAMPLE_PROBABILITY="${DECISION_TRACING_SAMPLE_PROBABILITY:-1.00}"
